...
def decomposed(factor,recovered,l):
    '''
    Input: 
...
for samples in range (2,34):
    ...
    for repeat in range(1,500):
        ...
        core95,factor95 = tucker(recovered95, ranks =  [recovered95.shape[0],1,l])
        # Tensor of each category is decompsed based on the factors obtained earlier
        _decomposed10 = decomposed(factor10, recovered10,l)
        _decomposed50 = decomposed(factor50, recovered50,l)
        _decomposed95 = decomposed(factor95, recovered95,l)
        
        # Switch to execute acc_vs_samples branch here
        if acc_vs_samples:
            t=np.random.randint((recovered10.shape[0]+recovered50.shape[0]+recovered95.shape[0]),size= samples)
        ...
        # Switch to execute acc_vs_reduced_samples branch
            ...
            _Y = np.ravel(np.array([[1]*recovered10.shape[0] + [2]*recovered50.shape[0] + [3]*recovered95.shape[0]]))
